...
OUTPUT_PATH = '/home/freshjinyong/techcapitallab/src/assets/images/portfolio-rebalancing-shannon-demon-nps-asset-allocation-20260914-og.png'
BG_PATH = '/home/freshjinyong/.gemini/antigravity-cli/brain/081927d3-9a7e-401e-819a-ab64e6efe38f/rebalance_bg_clean_1789388554309.jpg'
...
# Base horizontal gradient (Ensures left 52% is dark & legible for typography)
for x in range(W):
    nx = x / W
    if nx <= 0.50:
        alpha = 255
...
# Financial tech grid texture (left 50%)
...
# --- [Header: TechCapitalLab | 가치투자 & 자산배분 클래스 • 2026.09.14] ---
top_y = 56 * SCALE
...
meta_text = "가치투자 & 자산배분 클래스  •  2026.09.14"
...
# --- [Badge Line: 변동성을 수익으로 바꾸는 마법 • 리밸런싱의 모든 것] ---
badg_y = 112 * SCALE
badg_pad_h = 24 * SCALE
badg_pad_w = 12 * SCALE

# Badge 1
b1_text = "변동성을 수익으로 바꾸는 마법"
...
# --- [Main Headline: 폭락장에 웃는 리밸런싱의 원리] ---
hl_y = 158 * SCALE
hl_text = "폭락장에 웃는 리밸런싱의 원리"
...
# --- [Subtitle: 섀넌의 도깨비 수학적 복리와 국민연금식 5% 룰] ---
sub_y = 238 * SCALE
sub_text = "섀넌의 도깨비 수학적 복리와 국민연금식 5% 룰"
...
# --- [Data Line: 변동성 50% 축소 | MDD 방어율 극대화 | 기계적 고점익절 시스템] ---
data_line_y = 416 * SCALE

def draw_styled_data_line(draw_ctx, start_x, y):
    parts = [
        ("변동성 50% 축소", (52, 211, 153, 255)),
        (" | ", (100, 116, 139, 200)),
        ("MDD 방어율 극대화", (56, 189, 248, 255)),
        (" | ", (100, 116, 139, 200)),
        ("기계적 고점익절 시스템", (251, 191, 36, 255))
    ]
    px = start_x
...
# --- [Tags at bottom] ---
tags_y = 486 * SCALE
tags = ["#자산배분", "#리밸런싱", "#섀넌의도깨비", "#국민연금5퍼센트룰", "#MDD방어", "#TechCapitalLab"]
tag_x = lx

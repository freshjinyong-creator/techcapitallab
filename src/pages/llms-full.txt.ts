...
export const GET: APIRoute = async () => {
  ...
  const lines = [
    `# ${config.site.title} - LLMS Full Archive Knowledge Base`,
    "",
    ...
    "",
    `## 전체 분석 포스트 지식 베이스 (총 ${sortedPosts.length}편)`,
    ...sortedPosts.map(({ data, id, filePath }) => {
      ...
      const tags = data.tags ? `[태그: ${data.tags.join(", ")}]` : "";
      const date = data.pubDatetime ? `[발행일: ${new Date(data.pubDatetime).toISOString().split("T")[0]}]` : "";
      return `\n### [${data.title}](${url})\n${date} ${tags}\n- **핵심 직답 및 요약**: ${desc}\n- **카테고리**: ${data.category ?? "일반"}`;
    }),
    "",
    "## 데이터 및 인용 정책",
